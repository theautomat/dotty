/**
 * CollisionManager.ts - Centralized collision detection
...
interface CheckCollisionsParams {
  camera?: THREE.Camera;
}
...
class CollisionManager {
  ...
  private playerDot: any | null;  // DOTTY: Player reference for 2D collision detection
  private powerUpManager: any | null;
  private levelManager: any | null;

  // Configuration
  private SHIP_COLLISION_RADIUS: number;
   ...
   * @param params - Optional parameters (primarily camera position)
   */
  checkCollisions(params: CheckCollisionsParams = {}): void {
    ...
    if (enemyProjectiles.length > 0 && this.camera) {
      this.checkPlayerEnemyProjectileCollisions(this.camera.position, enemyProjectiles);
    }

    // Check player-enemy collisions
    if (enemies.length > 0 && this.camera) {
      this.checkPlayerEnemyCollisions(this.camera.position, enemies);
    }
  }
   ...
   */
  checkCollectibleCollisions(playerPosition: THREE.Vector3): void {
   ...
   * @param playerPosition - The position of the player
   * @param asteroids - The asteroids to check against
   */
  checkPlayerAsteroidCollisions(playerPosition: THREE.Vector3, asteroids: any[]): void {
   ...
   * @param asteroids - The asteroids to check
   */
  checkBulletAsteroidCollisions(bullets: any[], asteroids: any[]): void {
  ...
  */
   ...
   * @param playerPosition - The position of the player
   * @param enemyProjectiles - The enemy projectiles to check
   */
  checkPlayerEnemyProjectileCollisions(playerPosition: THREE.Vector3, enemyProjectiles: any[]): void {
   ...
   * @param playerPosition - The position of the player
   * @param enemies - The enemies to check against
   */
  checkPlayerEnemyCollisions(playerPosition: THREE.Vector3, enemies: any[]): void {
