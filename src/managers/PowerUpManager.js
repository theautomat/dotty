/**
 * PowerUpManager.js - Centralized power-up management system
 * 
...
class PowerUpManager {
     ...
     * @param {THREE.Vector3} position - Position where power-up should be created
     * @param {string} [specificType] - Optional type of power-up to create
     * @returns {PowerUp} The created PowerUp instance
     */
    createPowerUp(position, specificType = null) {
     ...
     * @param {PowerUp} powerUp - The power-up to remove
     */
     ...
     * @returns {number} Number of power-ups
     */
    getPowerUpCount() {
     ...
     */
    trySpawnPowerUp(position, levelConfig) {
        // Log the incoming position
     ...
     * @returns {Object|null} The power-up effect data or null if not active
     */
    getPowerUp(type) {
     ...
     * @returns {Object} Map of active power-ups
     */
    getAllActivePowerUps() {
     ...
     * @param {string} type - The power-up type to remove
     */
     ...
     * @param {boolean} isActivation - True if activating, false if deactivating
     * @private
     */
    _applySpecificEffect(type, effect, isActivation) {
        if (!this.game) return;
     ...
     * @param {number} collectionRadius - How close the player needs to be to collect the power-up
     * @returns {PowerUp|null} The collected power-up or null if none was collected
     */
    collectNearbyPowerUp(playerPosition, collectionRadius = GameConfig.powerUps.collectionRadius) {
