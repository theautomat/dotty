/**
 * WorldUtils.ts - Utility functions for world boundary and spatial operations
 *
 ...
 * @param extraMargin - Optional additional margin beyond the default boundary margin
 * @returns True if the position is outside the boundary
 */
export function isOutsideWorldBoundary(position: THREE.Vector3, extraMargin: number = 0): boolean {
 ...
 */
export function distanceToBoundary(position: THREE.Vector3): number {
  const halfWorldSize = GameConfig.world.size / 2;
 ...
 *
 * @param position - The position to check
 * @returns Value from 0-1 representing percentage to boundary (1 = at boundary)
 */
export function boundaryPercentage(position: THREE.Vector3): number {
  const halfWorldSize = GameConfig.world.size / 2;
