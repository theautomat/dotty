/**
 * ShieldEffect.ts - Visual shield bubble effect for the player's ship
 *
...
class ShieldEffect extends GameObject {
     ...
     * @param player - The player ship object
     * @param duration - Duration of the shield effect in ms
     */
    constructor(scene: THREE.Scene, player: Player, duration: number = 40000) {
        super(scene);
     ...
     */
    private printShieldParameters(): void {
        if (!this.mesh) return;
     ...
     * @returns Whether the shield is still active
     */
    update(): boolean {
        ...
        if (this.mesh && this.mesh.material instanceof THREE.MeshBasicMaterial && remainingPercentage < 0.3) {
