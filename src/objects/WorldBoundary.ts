...
class WorldBoundary {
    ...
    private outOfBoundsTime: number;
    private outOfBoundsWarningActive: boolean;
    private outOfBoundsDeathTimer: number;
    private outOfBoundsLastFlash: number;
    private outOfBoundsFlashInterval: number;

    constructor(scene: THREE.Scene, size: number = GameConfig.world.size) {
        this.scene = scene;
        this.size = size;
        this.visible = false;
    ...
    private addLine(start: THREE.Vector3, end: THREE.Vector3, material: THREE.LineBasicMaterial): void {
     ...
     * @param hud - Reference to the HUD for flashing the screen
     * @returns True if player is outside boundary and should die
     */
    checkOutOfBounds(playerPosition: THREE.Vector3, hud: any): boolean {
