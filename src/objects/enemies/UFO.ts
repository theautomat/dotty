/**
 * UFO.ts - Implementation of UFO enemy type
...
type UFOState = 'emerging' | 'approaching' | 'hovering' | 'attacking';
...
class UFO extends Enemy {
    private soundEffectInterval: number;
    private lastSoundEffectTime: number;
    private uniqueId: string;
    private state: UFOState;
    private stateChangeTime: number;
    private movementProfile: MovementProfile;
    private distanceProfile: DistanceProfile;
    private hoverProfile: HoverProfile;
    ...
    private dimensionalPhase?: number;
    private glitchIntensity?: number;
    private glitchSpeed?: number;
    private lastGlitchTime?: number;
    private glitchDuration?: number;
    private isVisible?: boolean;

    constructor(scene: THREE.Scene, position: THREE.Vector3, params: any = {}) {
     ...
     */
    private updateDimensionalGlitch(): void {
        if (!this.lastGlitchTime || !this.glitchSpeed || !this.dimensionalPhase || !this.glitchIntensity) return;

        const now = performance.now();
        const timeSinceLastGlitch = now - this.lastGlitchTime;

        // If we're in a glitch period
        if (this.glitchDuration && this.glitchDuration > 0) {
     ...
     */
    private setState(newState: UFOState): void {
        this.state = newState;
        this.stateChangeTime = performance.now();
     ...
     */
    private updateSpeedForState(state: UFOState): void {
        switch (state) {
            case 'emerging':
     ...
     */
    private emergeFromEdge(playerPosition: THREE.Vector3): void {
     ...
     */
    private approachPlayer(playerPosition: THREE.Vector3): void {
        const direction = new THREE.Vector3().subVectors(
            playerPosition,
            this.mesh.position
        ).normalize();
     ...
     */
    private hoverAroundPlayer(playerPosition: THREE.Vector3): void {
     ...
     */
    private attackPlayer(playerPosition: THREE.Vector3): void {
     ...
     */
    static create(scene: THREE.Scene, position: THREE.Vector3, params: any = {}): UFO {
